...
- The only quality gate is `pnpm check` (tsc for client + server, then `node --test test/*.test.ts`). It must pass before you finish.
...
- Do NOT touch: `.env*`, `dev/`, `drizzle/`, `Dockerfile`, `compose.yaml`, `.github/`.
- Do not add dependencies unless the issue explicitly asks for it.
- Do not run `git commit` / `git push`; the pipeline handles git.
